...
/**
	...
	exchange char blocks.
*/
public class TestCharBlockOps extends ATestCharOps
{	
	...
	*/
	private void writeBlock(char [] x, int op_size, IIndicatorWriteFormat write)throws IOException
	{
		...
		for(int i=0;i<x.length;)
		{
			...
			write.writeCharBlock(x, i, transfer);
			i+=transfer;			
		};
	...
	*/
	private void expectBlockAccurate(char [] x, int op_size, IIndicatorReadFormat read)throws IOException
	{
		...
		final char [] y = new char [ x.length ];	//buffer, size we try out section reads.
		for(int i=0;i<x.length;)
		{
			...
			int r = read.readCharBlock(y, i, transfer);
			if (r!=transfer)
	...
	*/
	private void expectBlockInaccurate(char [] x, int op_size, IIndicatorReadFormat read)throws IOException
	{
		...
		final char [] y = new char [ x.length +op_size ];	//buffer, size we try out section reads.
