...
/**
	...
	are stable operations when interacting float operations
*/
public class TestNextSkipGetOpsFloat extends ATestFloatOps
{
	@Test public void testNextInitialData_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeType(TIndicator.TYPE_FLOAT);
			p.write.writeFloat((float)0.123e-3);
		p.write.writeFlush(TIndicator.FLUSH_FLOAT);
		p.write.writeType(TIndicator.TYPE_FLOAT);
			p.write.writeFloat((float)-1.123e-3);
		p.write.writeFlush(TIndicator.FLUSH_FLOAT);
	...
	@Test public void testNextInitialData_d()throws IOException
	{
		...
		*/
		...
		p.read.open();
		assertGetIndicator(p.read,TIndicator.TYPE_FLOAT);
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		p.read.next();
		assertReadFlushIndicator(p.read,TIndicator.FLUSH_FLOAT);
	...
	@Test public void testNextDataBlock_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final float [] data = createSequence(757);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_FLOAT_BLOCK);
					p.write.writeFloatBlock(data);
	...
	@Test public void testNextDataBlock_d()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final float [] data = createSequence(157);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_FLOAT_BLOCK);
					p.write.writeFloatBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_FLOAT_BLOCK);
		...
		Assert.assertTrue("A".equals(p.read.getSignalName()));
		p.read.next();
		assertGetIndicator(p.read,TIndicator.TYPE_FLOAT_BLOCK);
			p.read.next();
			assertGetIndicator(p.read,TIndicator.DATA);
			//skip data
			p.read.next();
			assertReadFlushIndicator(p.read,TIndicator.FLUSH_FLOAT_BLOCK);				
		assertGetIndicator(p.read,TIndicator.END);
		assertGetIndicator(p.read,TIndicator.END);
		p.read.next();
		assertGetIndicator(p.read,TIndicator.EOF);
	...
	@Test public void testNextDataBlockRd_ud()throws IOException
	{
		...
		*/
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final float [] y = new float [data.length/2];
		Assert.assertTrue(p.read.readFloatBlock(y)==y.length); 
		//skip remaning data
		p.read.next();		
		assertGetIndicator(p.read,TIndicator.END);
		assertGetIndicator(p.read,TIndicator.END);
	...
	@Test public void testNextDataBlockRdf_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final float [] data = createSequence(1757);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_FLOAT_BLOCK);
					p.write.writeFloatBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_FLOAT_BLOCK);
	...
	@Test public void testNextDataBlockRdp_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final float [] data = createSequence(7);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_FLOAT_BLOCK);
					p.write.writeFloatBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_FLOAT_BLOCK);
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final float [] y = new float [data.length+5];
		Assert.assertTrue(p.read.readFloatBlock(y)==data.length);
		//Now cursor should be at the END
		assertGetIndicator(p.read,TIndicator.END);
		//And skip should be no-operation 
		p.read.skip();		
	...
	@Test public void testNextDataBlockRdp_d()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final float [] data = createSequence(256);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_FLOAT_BLOCK);
					p.write.writeFloatBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_FLOAT_BLOCK);
