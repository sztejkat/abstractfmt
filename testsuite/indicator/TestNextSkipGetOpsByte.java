...
/**
	...
	are stable operations when interacting byte operations
*/
public class TestNextSkipGetOpsByte extends ATestByteOps
{
	@Test public void testNextInitialData_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeType(TIndicator.TYPE_BYTE);
			p.write.writeByte((byte)84);
		p.write.writeFlush(TIndicator.FLUSH_BYTE);
		p.write.writeType(TIndicator.TYPE_BYTE);
			p.write.writeByte((byte)-3);
		p.write.writeFlush(TIndicator.FLUSH_BYTE);
	...
	@Test public void testNextInitialData_d()throws IOException
	{
		...
		*/
		...
		//We write some number of primitives.
		p.write.writeType(TIndicator.TYPE_BYTE);
			p.write.writeByte((byte)111);
		p.write.writeFlush(TIndicator.FLUSH_BYTE);
		p.write.writeType(TIndicator.TYPE_BYTE);
			p.write.writeByte((byte)-123);
		p.write.writeFlush(TIndicator.FLUSH_BYTE);
		p.write.writeBeginDirect("A");
		p.write.writeEnd();		
		p.write.flush();
		...
		p.read.open();
		assertGetIndicator(p.read,TIndicator.TYPE_BYTE);
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		p.read.next();
		assertReadFlushIndicator(p.read,TIndicator.FLUSH_BYTE);
	...
	@Test public void testNextDataBlock_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final byte [] data = createSequence(757);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_BYTE_BLOCK);
					p.write.writeByteBlock(data);
	...
	@Test public void testNextDataBlock_d()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final byte [] data = createSequence(157);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_BYTE_BLOCK);
					p.write.writeByteBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_BYTE_BLOCK);
		...
		Assert.assertTrue("A".equals(p.read.getSignalName()));
		p.read.next();
		assertGetIndicator(p.read,TIndicator.TYPE_BYTE_BLOCK);
			p.read.next();
			assertGetIndicator(p.read,TIndicator.DATA);
			//skip data
			p.read.next();
			assertReadFlushIndicator(p.read,TIndicator.FLUSH_BYTE_BLOCK);				
		assertGetIndicator(p.read,TIndicator.END);
		assertGetIndicator(p.read,TIndicator.END);
		p.read.next();
		assertGetIndicator(p.read,TIndicator.EOF);
	...
	@Test public void testNextDataBlockRd_ud()throws IOException
	{
		...
		*/
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final byte [] y = new byte [data.length/2];
		Assert.assertTrue(p.read.readByteBlock(y)==y.length); 
		//skip remaning data
		p.read.next();		
		assertGetIndicator(p.read,TIndicator.END);
		assertGetIndicator(p.read,TIndicator.END);
	...
	@Test public void testNextDataBlockRdf_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final byte [] data = createSequence(1757);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_BYTE_BLOCK);
					p.write.writeByteBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_BYTE_BLOCK);
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final byte [] y = new byte [data.length];
		Assert.assertTrue(p.read.readByteBlock(y)==y.length);
	...
	@Test public void testNextDataBlockRdp_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final byte [] data = createSequence(7);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_BYTE_BLOCK);
					p.write.writeByteBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_BYTE_BLOCK);
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final byte [] y = new byte [data.length+5];
		Assert.assertTrue(p.read.readByteBlock(y)==data.length);
		//Now cursor should be at the END
		assertGetIndicator(p.read,TIndicator.END);
		//And skip should be no-operation 
		p.read.skip();		
	...
	@Test public void testNextDataBlockRdp_d()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final byte [] data = createSequence(256);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_BYTE_BLOCK);
					p.write.writeByteBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_BYTE_BLOCK);
