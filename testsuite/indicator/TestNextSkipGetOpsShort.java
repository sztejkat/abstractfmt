...
/**
	...
	are stable operations when interacting short operations
*/
public class TestNextSkipGetOpsShort extends ATestShortOps
{
	@Test public void testNextInitialData_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeType(TIndicator.TYPE_SHORT);
			p.write.writeShort((short)0x3345);
		p.write.writeFlush(TIndicator.FLUSH_SHORT);
		p.write.writeType(TIndicator.TYPE_SHORT);
			p.write.writeShort((short)0x0011);
		p.write.writeFlush(TIndicator.FLUSH_SHORT);
	...
	@Test public void testNextInitialData_d()throws IOException
	{
		...
		*/
		...
		p.read.open();
		assertGetIndicator(p.read,TIndicator.TYPE_SHORT);
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		p.read.next();
		assertReadFlushIndicator(p.read,TIndicator.FLUSH_SHORT);
	...
	@Test public void testNextDataBlock_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final short [] data = createSequence(757);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_SHORT_BLOCK);
					p.write.writeShortBlock(data);
	...
	@Test public void testNextDataBlock_d()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final short [] data = createSequence(157);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_SHORT_BLOCK);
					p.write.writeShortBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_SHORT_BLOCK);
		...
		Assert.assertTrue("A".equals(p.read.getSignalName()));
		p.read.next();
		assertGetIndicator(p.read,TIndicator.TYPE_SHORT_BLOCK);
			p.read.next();
			assertGetIndicator(p.read,TIndicator.DATA);
			//skip data
			p.read.next();
			assertReadFlushIndicator(p.read,TIndicator.FLUSH_SHORT_BLOCK);				
		assertGetIndicator(p.read,TIndicator.END);
		assertGetIndicator(p.read,TIndicator.END);
		p.read.next();
		assertGetIndicator(p.read,TIndicator.EOF);
	...
	@Test public void testNextDataBlockRd_ud()throws IOException
	{
		...
		*/
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final short [] y = new short [data.length/2];
		Assert.assertTrue(p.read.readShortBlock(y)==y.length); 
		//skip remaning data
		p.read.next();		
		assertGetIndicator(p.read,TIndicator.END);
		assertGetIndicator(p.read,TIndicator.END);
	...
	@Test public void testNextDataBlockRdf_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final short [] data = createSequence(1757);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_SHORT_BLOCK);
					p.write.writeShortBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_SHORT_BLOCK);
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final short [] y = new short [data.length];
		Assert.assertTrue(p.read.readShortBlock(y)==y.length);
	...
	@Test public void testNextDataBlockRdp_ud()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final short [] data = createSequence(7);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_SHORT_BLOCK);
					p.write.writeShortBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_SHORT_BLOCK);
		...
		p.read.next();
		assertGetIndicator(p.read,TIndicator.DATA);
		final short [] y = new short [data.length+5];
		Assert.assertTrue(p.read.readShortBlock(y)==data.length);
		//Now cursor should be at the END
		assertGetIndicator(p.read,TIndicator.END);
		//And skip should be no-operation 
		p.read.skip();		
	...
	@Test public void testNextDataBlockRdp_d()throws IOException
	{
		...
		*/
		...
		p.write.writeBeginDirect("A");		
				...
				final short [] data = createSequence(256);
				//We write some number of primitives.
				p.write.writeType(TIndicator.TYPE_SHORT_BLOCK);
					p.write.writeShortBlock(data);
				p.write.writeFlush(TIndicator.FLUSH_SHORT_BLOCK);
