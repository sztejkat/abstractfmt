...
/**
	Tests if int blocks are properly processed,
...
public class TestIntBlockOps extends ATestIntOps
{
	...
	@Test public void fullBlockWriteRead()throws IOException
	{
		...
		*/
		enter();
		Pair p = create();
		int [] sequence = createSequence(100);
		p.write.open();
		p.write.begin("array");
		p.write.writeIntBlock(sequence);
		p.write.end();
		p.write.close();		
	...
	private void scatterBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			...
			assertWhatNext(p.read,TContentType.PRMTV_INT_BLOCK);
			int [] x = new int[t+10];
			int r = p.read.readIntBlock(x,10,t);
	...
	private void scatterPartialBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			...
			assertWhatNext(p.read,TContentType.PRMTV_INT_BLOCK);
			int [] x = new int[transfer_size];
			int r = p.read.readIntBlock(x,0,transfer_size);
	...
	@Test public void testZeroOperation()throws IOException
	{
		...
		*/
		...
		p.write.open();
		p.write.begin("array");
		p.write.writeIntBlock(new int[0]);
		p.write.end();
		p.write.close();		
