...
/**
	Tests if double blocks are properly processed,
...
public class TestDoubleBlockOps extends ATestDoubleOps
{
	...
	@Test public void fullBlockWriteRead()throws IOException
	{
		...
		*/
		enter();
		Pair p = create();
		double [] sequence = createSequence(100);
		p.write.open();
		p.write.begin("array");
		p.write.writeDoubleBlock(sequence);
		p.write.end();
		p.write.close();		
		...
		double [] x = new double[sequence.length];
		int r = p.read.readDoubleBlock(x);
	...
	private void scatterBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			int t = Math.min(transfer_size,sequence.length-i);
			assertWhatNext(p.read,TContentType.PRMTV_DOUBLE_BLOCK);
			double [] x = new double[t+10];
			int r = p.read.readDoubleBlock(x,10,t);
	...
	private void scatterPartialBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			...
			assertWhatNext(p.read,TContentType.PRMTV_DOUBLE_BLOCK);
			double [] x = new double[transfer_size];
			int r = p.read.readDoubleBlock(x,0,transfer_size);
	...
	@Test public void testZeroOperation()throws IOException
	{
		...
		*/
		...
		p.write.open();
		p.write.begin("array");
		p.write.writeDoubleBlock(new double[0]);
		p.write.end();
		p.write.close();		
