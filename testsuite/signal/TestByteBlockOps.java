...
/**
	Tests if byte blocks are properly processed,
...
public class TestByteBlockOps extends ATestByteOps
{
	...
	@Test public void fullBlockWriteRead()throws IOException
	{
		...
		*/
		enter();
		Pair p = create();
		byte [] sequence = createSequence(100);
		p.write.open();
		p.write.begin("array");
		p.write.writeByteBlock(sequence);
		p.write.end();
		p.write.close();		
		...
		byte [] x = new byte[sequence.length];
		int r = p.read.readByteBlock(x);
	...
	private void scatterBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			int t = Math.min(transfer_size,sequence.length-i);
			assertWhatNext(p.read,TContentType.PRMTV_BYTE_BLOCK);
			byte [] x = new byte[t+10];
			int r = p.read.readByteBlock(x,10,t);
	...
	private void scatterPartialBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			...
			assertWhatNext(p.read,TContentType.PRMTV_BYTE_BLOCK);
			byte [] x = new byte[transfer_size];
			int r = p.read.readByteBlock(x,0,transfer_size);
	...
	@Test public void testZeroOperation()throws IOException
	{
		...
		*/
		...
		p.write.open();
		p.write.begin("array");
		p.write.writeByteBlock(new byte[0]);
		p.write.end();
		p.write.close();		
	...
	@Test public void fullByte_by_byte_BlockWriteRead()throws IOException
	{
		/*
			We test if we can perform a byte-by-byte
			block write and read it in one operation
		*/
		enter();
		Pair p = create();
		byte [] sequence = createSequence(100);
		...
		for(int i=0;i<sequence.length;i++)
			p.write.writeByteBlock(sequence[i]);
		p.write.end();
		p.write.close();		
	...
	@Test public void fullBlockWrite_byte_byte_Read()throws IOException
	{
		/*
			We test if we can perform a block write and read it byte-by-byte
		*/
		enter();
		Pair p = create();
		byte [] sequence = createSequence(100);
		...
		{
			int r = p.read.readByteBlock();				
			Assert.assertTrue(r>=0);
			Assert.assertTrue(r<=255);
			x[i]=(byte)r;
		};
		assertEqual(x,0,x.length,sequence,0);
		assertWhatNext(p.read,TContentType.SIGNAL);
		assertNext(p.read,null);
		assertWhatNext(p.read,TContentType.EOF);
		p.read.close();
	...
	@Test public void fullBlockWrite_byte_byte_ReadEndOf()throws IOException
	{
		/*
			We test if we can perform a full block 
			write and read back byte-by-byte.
		*/
		enter();
		Pair p = create();
		byte [] sequence = createSequence(100);
	...
	@Test public void fullBlockWrite_byte_byte_ReadZeroData()throws IOException
	{
		/*
			We test if we can perform a zero block 
			write and read back byte-by-byte.
		*/
		enter();
		Pair p = create();
