...
/**
	Tests if long blocks are properly processed,
...
public class TestLongBlockOps extends ATestLongOps
{
	...
	@Test public void fullBlockWriteRead()throws IOException
	{
		...
		*/
		enter();
		Pair p = create();
		long [] sequence = createSequence(100);
		p.write.open();
		p.write.begin("array");
		p.write.writeLongBlock(sequence);
		p.write.end();
		p.write.close();		
		...
		long [] x = new long[sequence.length];
		int r = p.read.readLongBlock(x);
	...
	private void scatterBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			int t = Math.min(transfer_size,sequence.length-i);
			assertWhatNext(p.read,TContentType.PRMTV_LONG_BLOCK);
			long [] x = new long[t+10];
			int r = p.read.readLongBlock(x,10,t);
	...
	private void scatterPartialBlockRead(int block_size, int transfer_size)throws IOException
	{
		...
		*/
		...
		for(int i=0;i<sequence.length;i+=transfer_size)
		{
			...
			assertWhatNext(p.read,TContentType.PRMTV_LONG_BLOCK);
			long [] x = new long[transfer_size];
			int r = p.read.readLongBlock(x,0,transfer_size);
	...
	@Test public void testZeroOperation()throws IOException
	{
		...
		*/
		...
		p.write.open();
		p.write.begin("array");
		p.write.writeLongBlock(new long[0]);
		p.write.end();
		p.write.close();		
